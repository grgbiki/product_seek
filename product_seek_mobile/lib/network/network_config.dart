class NetworkConfig {
  /// Login API Keys
  static const String API_KEY_USER_ID = "id";
  static const String API_KEY_USER_NAME = "name";
  static const String API_KEY_USER_EMAIL = "email";
  static const String API_KEY_USER_PASSWORD = "password";
  static const String API_KEY_USER_PASSWORD_CONFIRMATION =
      "password_confirmation";
  static const String API_KEY_USER_PHONE_NUMBER = "phone_number";
  static const String API_KEY_USER_ADDRESS = "address";
  static const String API_KEY_USER_ROLE = "role";

  static const String API_KEY_USER_OLD_PASSWORD = "old_password";
  static const String API_KEY_USER_NEW_PASSWORD = "new_password";
  static const String API_KEY_USER_NEW_PASSWORD_CONFIRMATION =
      "new_password_confirmation";

  static const String API_KEY_LOGIN_RESPONSE = "response";

  static const String API_KEY_PRODUCT_ID = "id";
  static const String API_KEY_PRODUCT_TITLE = "title";
  static const String API_KEY_PRODUCT_PRICE = "price";
  static const String API_KEY_PRODUCT_DESCRIPTION = "description";
  static const String API_KEY_PRODUCT_WARRENTY = "warrenty";
  static const String API_KEY_PRODUCT_IMAGE = "product_image";
  static const String API_KEY_PRODUCT_CATEGORY = "product_category";
  static const String API_KEY_PRODUCT_CATEGORY_ID = "product_category_id";
  static const String API_KEY_PRODUCT_STORE = "product_store";
  static const String API_KEY_PRODUCT_STORE_ID = "product_store_id";

  static const String API_KEY_STORE_ID = "id";
  static const String API_KEY_STORE_NAME = "name";
  static const String API_KEY_STORE_EMAIL = "email";
  static const String API_KEY_STORE_CONTACT = "contact";
  static const String API_KEY_STORE_ADDRESS = "address";
  static const String API_KEY_STORE_FOLLOWERS = "followers";
  static const String API_KEY_STORE_MAP_URL = "google_maps_url";

  static const String API_KEY_CATEGORY_ID = "id";
  static const String API_KEY_CATEGORY_NAME = "name";

  static const String API_KEY_FEEDBACK_USER_ID = "user_id";
  static const String API_KEY_FEEDBACK = "feedback";

  static const String API_KEY_WISHLIST_ID = "id";
  static const String API_KEY_WISHLIST_PRODUCT = "product";
  static const String API_KEY_WISHLIST_PRODUCT_ID = "product_id";
  static const String API_KEY_WISHLIST_USER_ID = "user_id";

  static const String API_KEY_ORDER_ID = "id";
  static const String API_KEY_ORDER_NUMBER = "order_no";
  static const String API_KEY_ORDER_PRODUCTS = "products";
  static const String API_KEY_ORDER_STATUS = "status";
  static const String API_KEY_ORDER_TOTAL = "total";
  static const String API_KEY_ORDER_QUANTITY = "quantity";
  static const String API_KEY_RETURN_ORDER_BODY = "return_note";
  static const String API_KEY_ORDER_RETURNABLE = "returnable";

  static const String API_KEY_REVIEW = "review";
  static const String API_KEY_REVIEW_ID = "id";
  static const String API_KEY_REVIEW_USER_ID = "user_id";
  static const String API_KEY_REVIEW_PRODUCT_ID = "product_id";
  static const String API_KEY_REVIEW_USERNAME = "username";
}
